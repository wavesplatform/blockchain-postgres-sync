...
begin
	...
	if b->>'reward' is not null then
		...
		values ((b->>'height')::integer, coalesce((select quantity from waves_data where height = (b->>'height')::integer - 1), (select quantity from waves_data where height = 0)) + (b->>'reward')::bigint) 
		on conflict do nothing;
	end if;
END
$function$
